...
export default class ConfigManager {
    public args = newprocessArgs([{ name: "config", type: "string", default: "default" }, { name: "watch", type: "boolean", optional: true }]);
    ...
    public envConfig = newprocessArgs([
        ...
        { name: "server-sentry-traces-sample-rate", type: "number", optional: true, newName: "server-sentry-tracesSampleRate" },
    ...
    private getPath(p?: string): string {
        ...
        return p ? join(this.configsPath, p.endsWith(".json") ? p : `${p}.json`) : join(this.configsPath, configPath.endsWith(".json") ? configPath : `${configPath}.json`);
    }
    ...
    public async load(path?: string, reload?: boolean): Promise<MySchema | null> {
        ...
        if (this.args.config === "default") {
            const read = fileExists ? await fs.readFile(pth, { encoding: "utf8" }) : await fs.readFile(this.examplePath, { encoding: "utf8" });
        ...
        try {
            ...
            if (result.valid) {
                ...
                App.StaticLogger.warn(`Config file is invalid, please fix it before continuing (${this.args.watch ? "The file is being watched" : "Please restart the process once you fix the issue, or use --watch to automatically reload the config"})`);
            }
    ...
    private compare(obj1: any, obj2: any, parentKey: number | string = ""): string[] {
        ...
        for (const key of allKeys) {
            ...
            if (typeof value1 === "object" && typeof value2 === "object" && value1 !== null && value2 !== null && !Array.isArray(value1) && !Array.isArray(value2)) {

...
class PermissionHandler {
    ...
    public constructor(
        ...
        Channels?: { Id: string, Overrides: { Allow: bigint | number | string, Deny: bigint | number | string, Id: string, Type: 'Member' | 'Role'; }[]; }[]
    ) {
    ...
    public CanManageRole(
        Role: {
            Permissions: bigint | number | keyof typeof PermissionConstants;
            Position: number,
        }
    ) {
    ...
    public HasChannelPermission(channelId: string, permission: keyof typeof PermissionConstants): boolean {
        ...
        const userOverride = channel.Overrides.find(override => override.Type === 'Member' && override.Id === this.GuildMemberId);
        if (userOverride) {
            if ((BigInt(userOverride.Allow) & PermissionConstants[permission]) === PermissionConstants[permission]) return true;
            if ((BigInt(userOverride.Deny) & PermissionConstants[permission]) === PermissionConstants[permission]) return false;
        }
        ...
        for (const role of this.MemberRoles) {
            ...
            if (roleOverride) {
                if ((BigInt(roleOverride.Allow) & PermissionConstants[permission]) === PermissionConstants[permission]) return true;
                if ((BigInt(roleOverride.Deny) & PermissionConstants[permission]) === PermissionConstants[permission]) return false;
            }
        }

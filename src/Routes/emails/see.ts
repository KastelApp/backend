...
const isRegisted = <T extends "BadActor" | "RecentLogin" | "Registration">(type: T, deaf: unknown): deaf is { default(username: string, verificationCode: string, deleteAccountUrl: string): JSX.Element; } => {
    return type === "Registration" && typeof deaf === "object" && deaf !== null && "default" in deaf && typeof deaf.default === "function";
};
...
export default class Index extends Route {
    ...
    public async Request({
        ...
        if (isRegisted(query.type, imported)) {
            const rendered = render(imported.default( query.username ?? "User", query.verificationCode ?? "https://example.com/verify/1234567890", query.deleteAccountUrl ?? "https://example.com/delete/1234567890"), {
                pretty: true
            });

...
export default class FetchGuilds extends Route {
	...
	public postGuild({ body, user }: CreateRoute<"/guilds", Infer<typeof postGuild>, [UserMiddlewareType]>) {
		...
		if (body.channels) {
			...
			for (const channel of sortedChannels) {
				...
				const foundParent = newChannels.find((c) => c.oldId === channel.parentId) ?? sortedChannels.find((c) => c.id === channel.parentId);
	...
	public canCreateGuild(flags: FlagFields, guildCount: number) {
		...
		if (!flags.has("IncreasedGuildCount100") || !flags.has("IncreasedGuildCount200") || !flags.has("IncreasedGuildCount500") && guildCount < Constants.settings.Max.GuildCount) {
			return {
				can: true,
		...
		return {
			can: false,
			max: flags.has("IncreasedGuildCount100") ? 100 : flags.has("IncreasedGuildCount200") ? 200 : flags.has("IncreasedGuildCount500") ? 500 : Constants.settings.Max.GuildCount
		}
	}
}

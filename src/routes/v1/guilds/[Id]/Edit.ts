...
import Permissions from '../../../../Utils/Classes/BitFields/Permissions.js';
...
new Route(
	...
	async (req, res) => {
		const { name, description, ownerId, maxMembers, twofaCode } = req.body as RequestBody;
		...
		if (ownerId) {
			...
			if (!NewOwner) {
				...
				Errors.AddError({
					Member: {
						Code: 'InvalidMember',
						Message: 'The member you are trying to transfer ownership to is not in the guild.',
					},
				});
			}
			...
			if (NewOwnerFlags.hasString('Owner')) {
				...
				Errors.AddError({
					Member: {
						Code: 'InvalidMember',
						Message: 'The member you are trying to transfer ownership to is already the owner.',
					},
				});
			}
